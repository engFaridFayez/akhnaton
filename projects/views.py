...
def employment_message_view(request):
    if request.method == 'POST':
        EmploymentMessage.objects.create(
            name=request.POST.get('name'),
            phone=request.POST.get('phone'),
            age=request.POST.get('age'),
            place_of_residence=request.POST.get('place_of_residence'),
            job_applied_for=request.POST.get('job_applied_for'),
            numbers_of_years_experience=request.POST.get('numbers_of_years_experience'),
            message=request.POST.get('message'),
            cv=request.FILES.get('cv'),
        )
        request.session['submitted'] = True
        return redirect('employment_success')

    return render(request, 'projects/employment.html')

def employment_success(request):
    if not request.session.get('submitted'):
        return redirect('employment')
    request.session.pop('submitted')
    return render(request, 'projects/employment_success.html')
